...
public class EvaluatorVisitor implements ParserVisitor
{
	...
	/** The function table for function and operator lookup */
	...
	public Object visit(ASTFunNode node, Object data) {
        ...
        // Allows overriding basic operators (e.g., +, -).
        String functionSymbol = node.getName();
        // Allows overloading (such as for subtraction and unary minus)
        String overloadedSymbol = functionSymbol + ":" + pfmc.getNumberOfParameters();
        if (funTab.containsKey(overloadedSymbol)) {
            pfmc = (PostfixMathCommandI) funTab.get(overloadedSymbol);
        } else if (funTab.containsKey(functionSymbol)) {
            pfmc = (PostfixMathCommandI) funTab.get(functionSymbol);
        }
